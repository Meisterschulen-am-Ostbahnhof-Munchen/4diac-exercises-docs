...
**Scalability compared to `Uebung_233_AX` (`ASR_MERGE_3`):** `ASR_MERGE_N` exists as an entire family (`ASR_MERGE_2..7`) – going from 2 to 3 sources (230 → 233) simply means adding another `AX_ASR_RF_TRIG` and replacing `ASR_MERGE_2` with `ASR_MERGE_3`, all within a single block. In contrast, `AX_LAST_2` has **no** higher-order siblings – there is no `AX_LAST_3` in the library. A third raw source can only be included by cascading two `AX_LAST_2` instances (feeding one's `OUT` into the next one's `IN1`/`IN2`). This still preserves correct last-write-wins semantics across all three sources: `AX_LAST_2` copies the selected value to `OUT.D1` **and** fires `OUT.E1`, so every change at any of the original sources propagates through the cascade as a fresh event — unlike a plain OR gate, no recency information is lost. The difference from `ASR_MERGE_N` is purely structural: one block per input count vs. N-1 cascaded instances of the same two-input block, not a difference in semantics.
