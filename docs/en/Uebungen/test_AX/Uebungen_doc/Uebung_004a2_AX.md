...
The subapplication `Uebung_004a2_AX.SUB` uses a `E_MERGE` function block to route two input events to a flip-flop input.
...
- **`E_MERGE`**: Type `E_MERGE`. This component has two event inputs (`EI1`, `EI2`) and one event output (`EO`). Regardless of which input receives an event, it is immediately forwarded to the corresponding output .
