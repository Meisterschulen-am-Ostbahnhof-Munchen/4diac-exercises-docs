# Uebung_061_AX: Pure Time-Controlled Sequence, 4 Channels (Adapter Version)

This article describes the logiBUS® exercise `Uebung_061_AX`. We build a purely time-driven step chain using AX adapter technology.
...
The subapplication `Uebung_061_AX` uses the sequencer `sequence_T_04_AX` (AX adapter variant), which cycles through 4 states (`S1` to `S4`) purely on elapsed time.

![Uebung_061_AX_network](./Uebung_061_AX_network.svg)

- **`sequence_T_04_AX`**: Manages the 4 steps; each transition happens once the respective `DT_Sx_Sy` parameter elapses.
