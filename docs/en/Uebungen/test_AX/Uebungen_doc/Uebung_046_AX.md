# Uebung_046_AX: Combined Event/Time Sequence Control, 8 Channels (Loop) (Adapter Version)

This article describes the logiBUS® exercise `Uebung_046_AX`. We build a combined event- and time-driven step chain using AX adapter technology.
...
The subapplication `Uebung_046_AX` uses the combined sequencer `sequence_ET_08_loop_AX`.

![Uebung_046_AX_network](./Uebung_046_AX_network.svg)

- **`sequence_ET_08_loop_AX`**: Starts on an event (`I1`), then advances the 8 steps on elapsed time.
