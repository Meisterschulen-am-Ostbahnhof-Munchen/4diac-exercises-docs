...
This exercise uses the function block `sequence_ET_04_loop`. Once the sequence is started by button **I1**, it continuously executes steps 1 to 4. After completing step 4, it automatically returns to step 1. The cycle is only interrupted and the machine stopped by pressing the reset button **I4**. This is ideal for continuous operation or periodic mechanical movements.
