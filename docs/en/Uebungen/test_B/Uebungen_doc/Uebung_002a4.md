...
The subapplication `Uebung_002a4.SUB` links two digital inputs to a digital output via a logic block.
...
- **`AND_2_BOOL`**: An instance of type `AND_2_BOOL` (from the IEC 61131 library). This block performs the logical AND operation specifically for Boolean values. It has two data inputs (`IN1`, `IN2`) and one data output (`OUT`). Like all standard logic blocks, it responds to an event at port `REQ` and signals completion at port `CNF`.
- **`DigitalOutput_Q1`**: An instance of type `logiBUS_QX`. This block controls the hardware output `Output_Q1`.
