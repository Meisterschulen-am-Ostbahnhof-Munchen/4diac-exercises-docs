...
The sub-application `Uebung_012a.SUB` uses the sub-app type `Uebung_012a_sub` to provide the storage logic modularly. The block `CbVtStatus` remains at the top level to update the entire page as needed.

### Typed Sub-Application: `Uebung_012a_sub`

This module bundles the input via `NumericValue_ID`, the conversion, NVS access, and the display feedback. It provides interfaces for the memory key (`KEY`) and the object ID (`u16ObjId`).
