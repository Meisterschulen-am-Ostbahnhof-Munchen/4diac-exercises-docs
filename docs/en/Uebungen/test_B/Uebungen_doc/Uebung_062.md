# Uebung_062: Pure Time-Controlled Sequence, 5 Channels

This article describes the logiBUS® exercise `Uebung_062`. We build a purely time-driven step chain.
...
The subapplication `Uebung_062` uses the sequencer `sequence_T_05`, which cycles through 5 states (`S1` to `S5`) purely on elapsed time.

![Uebung_062_network](./Uebung_062_network.svg)

- **`sequence_T_05`**: Manages the 5 steps; each transition happens once the respective `DT_Sx_Sy` parameter elapses.
