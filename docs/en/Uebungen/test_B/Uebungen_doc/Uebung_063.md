# Uebung_063: Pure Time-Controlled Sequence, 8 Channels

This article describes the logiBUS® exercise `Uebung_063`. We build a purely time-driven step chain.
...
The subapplication `Uebung_063` uses the sequencer `sequence_T_08`, which cycles through 8 states (`S1` to `S8`) purely on elapsed time.

![Uebung_063_network](./Uebung_063_network.svg)

- **`sequence_T_08`**: Manages the 8 steps; each transition happens once the respective `DT_Sx_Sy` parameter elapses.
