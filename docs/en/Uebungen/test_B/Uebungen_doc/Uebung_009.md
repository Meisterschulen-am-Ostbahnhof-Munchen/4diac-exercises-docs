...
In `Uebung_009.SUB`, a clock generator is used to control an up counter, the value of which is sent to an ISOBUS terminal.
...
- **`E_PERMIT`**: An event gate. It only allows events at input `EI` to pass to output `EO` if the data input `PERMIT` is set to `TRUE`.
