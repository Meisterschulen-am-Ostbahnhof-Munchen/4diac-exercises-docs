...
- **ASR_SPLIT_2**: Verteiler für den einen ASR-Stream
    ...
    - **Parameter**: keine
    - **Erklärung**: Ein Adapter-Plug (`AX_ASR_RF_TRIG_1.Q`) ist strikt punkt-zu-punkt — er kann nicht direkt gleichzeitig an zwei `ASR_AX_SR.S_R`-Sockets angeschlossen werden. Genau dieselbe Grundregel, die in `Uebung_229_AX.md`/`Uebung_230_AX.md` `ASR_MERGE_2` nötig machte (mehrere Quellen können nicht direkt auf einen Socket), erzwingt hier umgekehrt `ASR_SPLIT_2` (eine Quelle kann nicht direkt zwei Plugs gleichzeitig bedienen). `ASR_SPLIT_2` dupliziert den einen ASR-Stream verlustfrei auf zwei Ausgänge.
