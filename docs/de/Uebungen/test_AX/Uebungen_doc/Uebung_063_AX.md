# Uebung_063_AX: Reine Zeitsteuerung 8-Kanal (Adapter Version)

Dieser Artikel beschreibt die logiBUS®-Übung `Uebung_063_AX`. Wir bauen eine rein zeitgesteuerte Schrittkette mit AX-Adaptertechnologie.
...
Die Subapplikation `Uebung_063_AX` nutzt den Sequenzer `sequence_T_08_AX` (AX-Adaptervariante), der 8 Zustände (`S1` bis `S8`) rein zeitgesteuert durchläuft.

![Uebung_063_AX_network](./Uebung_063_AX_network.svg)

- **`sequence_T_08_AX`**: Verwaltet die 8 Schritte, jeder Übergang erfolgt nach Ablauf der jeweiligen `DT_Sx_Sy`-Parameter.
